...
mysql_select_db("a_propos") or die("cette base de donne n existe pas");
  ...
  $output='';
   ...
   $query=mysql_query("SELECT * FROM moniteurs WHERE nom_M LIKE '%$searchq%' OR prenom_M LIKE '%$searchq%'") or die ("could not search!");   
   
   $count = mysql_num_rows($query);
     if($count == 0){ ?>
              <p class="couleur"> Aucun Résultat Trouvé </p>
			  <?php 
			try
{
...
$reponse = $bdd->query('SELECT idM,nom_M , prenom_M ,adresse_M,email_M,numero_telph  FROM moniteurs ORDER BY idM DESC LIMIT 0,20 ');
			?>
			    <table cellspacing='0' > 
	                <tr>
					    <th>Nom</th>
                    ...
                    <tr class='even'>
						...
						<td><?php echo $donnees['email_M']; ?></td>
                ...
                <?php    }else{
                 ...
                 while($rows = mysql_fetch_array($query)){
				                             ...
				                             $fname=$rows['nom_M'];
											  $lname=$rows['prenom_M'];
											  $adresse=$rows['adresse_M'];
											  $numt=$rows['numero_telph'];
											   $email=$rows['email_M'];
                    ...
                    <tr class='even'>
						...
						<td><a href="suppression2.php?idPersonne=<?php echo $id ;?>" onClick="return confirm('Voulez vous vraiment supprimer ce moniteur?');"><img src="images/corbeille.png"/></a></td>
						<td><a href="modifier_moniteur1.php?idPersonne=<?php echo $id;?>"><img src="images/url.jpg"/></a></td>
						<td><a href="envoyer_message_moniteur.php?idPersonne=<?php echo $id;?>"><img src="images/mail.jpg"/></a></td>
					</tr>
				 <?php	}
                            }
